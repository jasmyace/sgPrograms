runMN <- function(dat,progDir,BUGSDir,i){
...
# -----------------------------------------------------------------------------------------------------------------------  
  ...
  inits <- function(){                                                             # initialize the gibbs sampler
    list(taunoise=rgamma(1,2),                                                     # pull from rand uni [0,1], positive
         ...
         pi1=rnorm(length(unique(data$lekCls)),1),
         beta00=rnorm(length(unique(data$zoneCls)),4),
         beta10=rnorm(length(unique(data$zoneCls)),4),
  ...
  parameters <- c("taunoise",
                  ...
                  "tauinv2a","tauinv2b")                                           # the obj to a samp from post dist'n
  ...
  bayes <- bugs(data=data,                                                         # data to feed to winbugs
                ...
                model.file=paste0(progDir,"/TripleLevelFixedEffectMZone.R"),       # winbugs code
                #debug=TRUE,
  ...
  # output results
  save(data,inits,parameters,bayes,CYear,freqs,file=paste0(outpDir,"/Model N MZone 9.RData"))
