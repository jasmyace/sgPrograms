runMM <- function(dat,progDir,BUGSDir){
  ...
  inits <- function(){                                                             # initialize the gibbs sampler
    list(taunoise=rgamma(1,2),                                                     # pull from rand uni [0,1], positive
         ...
         tauinv3a=rgamma(1,2),
         tauinv3b=rgamma(1,2),
         tauinv2a=rgamma(length(unique(data$lekCls)),2),
         tauinv2b=rgamma(length(unique(data$lekCls)),2),
         gamma000=rnorm(1),
         gamma100=rnorm(1)
    )
  }
  ...
  parameters <- c("taunoise",
                  ...
                  "tauinv3a","tauinv3b","tauinv2a","tauinv2b")                     # the obj to a samp from post dist'n
  ...
  bayes <- bugs(data=data,                                                         # data to feed to winbugs
                ...
                model.file=paste0(progDir,"/TripleLevel.R"),                       # winbugs code
                debug=TRUE,
