makeRandomLekPlots <- function(dat,tracDir,file,bayes,bsums90){
  ...
  for(j in 1:10){
    ...
    lekSamp <- data.frame(lekCls=sample(unique(bsums90$lekCls[!is.na(bsums90$lekCls)]),16,replace=FALSE))   
    ...
    for(i in 1:20){
      ...
      if(i %% 5 == 1){
        ...
        if(length(unique(dat$Year)) == 11){
          backbone <- data.frame(Year=seq(2005,2015))
        ...
        if(i == 17){
          ...
          plot(X,yObs,type='p',pch=19,cex=0.8,col='gray50',axes=FALSE,frame.plot=TRUE,xlim=XLims,ylim=c(0,yMax),xlab='',ylab='')
          par(new=TRUE)
          plot(X,yEst,type='l',col='red',axes=FALSE,xpd=NA,frame.plot=TRUE,xlim=XLims,ylim=c(0,yMax),xlab='Year',ylab='',lwd=2)      
          axis(side=1,at=XLabs,labels=XLabs,cex.axis=0.8)
        } else {
          plot(X,yObs,type='p',pch=19,cex=0.8,col='gray50',axes=FALSE,frame.plot=TRUE,xlim=XLims,ylim=c(0,yMax),xlab='',ylab='')
          par(new=TRUE)
          plot(X,yEst,type='l',col='red',axes=FALSE, frame.plot=TRUE,xlim=XLims,ylim=c(0,yMax),xlab='',ylab='',lwd=2)              
        }
        leks <- leks[-1]
      } 
    }
  ...
  #rm(backbone,bsums90,bsums90Samp,lekSamp,forPlot,theSamp,thisSamp,mu.a,mu.b,nLeksI,nLeksS,theLek,xMax,xMin,X,yEst,yObs,yMin,yMax)
}