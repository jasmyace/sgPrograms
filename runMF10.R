runMF10 <- function(dat,progDir,BUGSDir,thisRun,z,j,burn,iter,thin){
  
  # check if there are any years missing data -- affects B matrix calculations if missing
  if(length(table(dat$Year)) != 11){
  ...
  data <- list(nCounts = length(dat$Peak_Males),                                   # number of peak_male counts
               ...
               medYear = 6,                                                       # median year index (constant) [1990]
  ...
  inits <- function(){                                                             # initialize the gibbs sampler
    list(B=array(rnorm(2*data$nLeks),c(data$nLeks,2)),                             # pull from multivariate normal 
         ...
         noise=rep(0.01,length(data$pMales))
    )
  }
  ...
  freqs <- seq(1964 + j,1964 + j + 10)  #glmer(Peak_Males ~ 1 + CYear + (1 + CYear | Lek_ID),data=countDat[[i]],family="poisson")
  
  # output results
  save(data,inits,parameters,bayes,CYear,freqs,time,file=paste0(outpDir,"/Model F MZone ",z," ",thisRun,".RData"))    
