runTestDn <- function(dat,progDir,BUGSDir,thisRun,z,burn,iter,thin){
  ...
  inits <- function(){                                                             # initialize the gibbs sampler
    list(a=rnorm(length(unique(dat$Lek_ID))),                                      # pull from nLeksx1 vector
         b=rnorm(length(unique(dat$Lek_ID))),                                      # pull from nLeksx1 vector                         
  ...
  bayes <- bugs(data=data,                                                         # data to feed to winbugs
                ...
                model.file=paste0(progDir,"/WBugsDn.R"),                           # winbugs code
                #debug=TRUE,
